...
async def _telegram_config() -> tuple[str, str, int, str]:
...
async def _configure_notifier(token: str, chat_id: str, seconds: int, language: str) -> None:
    sender = TelegramSender(token, chat_id).send if token and chat_id else None
    notifier.configure(seconds, sender, language)
    logger.info("notifier_configured", extra={"telegram": bool(sender)})
...
async def lifespan(app: FastAPI):
    ...
    token, chat_id, seconds, language = await _telegram_config()
    await _configure_notifier(token, chat_id, seconds, language)
